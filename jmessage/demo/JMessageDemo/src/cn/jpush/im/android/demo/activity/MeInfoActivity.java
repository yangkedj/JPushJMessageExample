...
public class MeInfoActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dialog.show();
        JMessageClient.getUserInfo(JMessageClient.getCurrentUser().getUserName(), new GetUserInfoCallback() {
            @Override
    ...
    public void StartModifyNickNameActivity() {
        String nickname = JMessageClient.getCurrentUser().getNickname();
    ...
    public void showSexDialog(final JMessageClient.Gender gender) {
        ...
        if (gender == JMessageClient.Gender.male) {
            manSelectedIv.setVisibility(View.VISIBLE);
            womanSelectedIv.setVisibility(View.GONE);
        } else if(gender == JMessageClient.Gender.female){
        ...
        View.OnClickListener listener = new View.OnClickListener() {
            ...
            public void onClick(View v) {
                switch (v.getId()) {
                    case R.id.man_rl:
                        if (gender != JMessageClient.Gender.male) {
                            JMessageClient.updateUserGender(JMessageClient.Gender.male, new BasicCallback() {
                                @Override
                        ...
                        break;
                    case R.id.woman_rl:
                        if (gender != JMessageClient.Gender.female) {
                            JMessageClient.updateUserGender(JMessageClient.Gender.female, new BasicCallback() {
                                @Override
                                public void gotResult(final int status, final String desc) {
                                    if (status == 0) {
                                        MeInfoActivity.this.runOnUiThread(new Runnable() {
    ...
    public void StartModifySignatureActivity() {
        Intent intent = new Intent();
        intent.putExtra("OldSignature", JMessageClient.getCurrentUser().getSignature());
