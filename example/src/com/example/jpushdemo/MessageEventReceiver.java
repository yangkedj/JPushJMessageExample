...
//	测试自动回复功能
	...
	public void onEvent(MessageEvent event) {
    	...
    	// 自动回复功能
    	if ( msg.getFromID().equals(SendFrom))
    	{
    		Conversation singleConvNew = Conversation.createConversation(ConversationType.single, SendFrom);
			
		       
			if (null == singleConvNew) {					
				Log.e(TAG_IM, "sendMessageSingle_targetId不存在");
			}else {
				TextContent singleContent = new TextContent("自动回复");
